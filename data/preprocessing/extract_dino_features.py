...
if __name__ == '__main__':
    ...
    # left camera frames
    frame_list = sorted(Path(scene_dir, 'color').glob('*'))
    save_dir = Path(scene_dir, 'dino')
    ...
    for frame_path in tqdm(frame_list):
        ...
        file_name = save_dir / f"{frame_name}.npy"
        if file_name.exists():
            continue
        frame, ori_h, ori_w = read_frame(str(frame_path))
